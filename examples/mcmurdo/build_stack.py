"""Build the McMurdo repeat-DEM stack on a common EPSG:3031 grid.

Stage 3 of the McMurdo pipeline. Consumes ASP-aligned DEM GeoTIFFs
...
def _stack_aoi_path() -> Path:
    ...
    return Path(config.MCMURDO_AOI_SHP)
...
def _qc_rejection_csv_for_variant(variant: str) -> Path | None:
    ...
    Returns the standard QC-list location used by ``mcmurdo.qc_strips``.
...
def plot_stack_coverage(stack: xr.DataArray, output_path: Path) -> None:
    ...
    fig.suptitle(
        f"McMurdo stack — {n} epochs on {stack.sizes['y']} x {stack.sizes['x']} grid",
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=140, bbox_inches="tight")
...
def main() -> xr.DataArray:
    ...
    out_nc = config.PROCESSED_DIR / f"mcmurdo_stack_{config.START_TIME}_{config.END_TIME}.nc"
    print(f"Saving stack -> {out_nc}")
    save_stack(stack, out_nc)

    print("Writing QC figures to", config.FIGURES_DIR)
    plot_stack_footprints(kept, config.MCMURDO_AOI_SHP, config.FIGURES_DIR / "stack_footprints.png")
