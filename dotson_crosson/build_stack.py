"""Build the Dotson + Crosson repeat-DEM stack on a common EPSG:3031 grid.

Stage 3 of the Dotson + Crosson pipeline. Consumes ASP-aligned DEM GeoTIFFs
...
Run directly:
    ...
    python -m dotson_crosson.build_stack
"""
...
def _qc_rejection_csv_for_variant(variant: str) -> Path | None:
    ...
    Returns the standard QC-list location used by ``dotson_crosson.qc_strips``.
...
def plot_stack_footprints(
    ...
    ax.set_title(
        f"Dotson + Crosson stack footprints\n"
...
def plot_stack_coverage(stack: xr.DataArray, output_path: Path) -> None:
    ...
    fig.suptitle(
        f"Dotson + Crosson stack — {n} epochs on {stack.sizes['y']} x {stack.sizes['x']} grid",
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=140, bbox_inches="tight")
...
def main() -> xr.DataArray:
    ...
    # clean 25-m lines. Also pad slightly so strips fully inside have edges.
    res = float(config.RES)
    ...
    out_nc = config.PROCESSED_DIR / f"dotson_crosson_stack_{config.START_TIME}_{config.END_TIME}.nc"
    print(f"Saving stack -> {out_nc}")
    save_stack(stack, out_nc)

    print("Writing QC figures to", config.FIGURES_DIR)
    plot_stack_footprints(kept, config.DC_AOI_SHP, config.FIGURES_DIR / "stack_footprints.png")
