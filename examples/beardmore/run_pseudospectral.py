"""Apply both pseudo-spectral linear-perturbation inverses to Beardmore.

Loads the existing Beardmore stack + floating mask + velocity, runs the
...
Separate from :mod:`beardmore.run_melt` (which runs the three existing
classic solvers) so the pseudo-spectral pipeline can be iterated
without disturbing the existing driver.

Run:

    python -m beardmore.run_pseudospectral
"""
...
def plot_pseudospectral_summary(
    ...
    fig.suptitle(
        f"Pseudo-spectral linear-perturbation inverses — Beardmore "
