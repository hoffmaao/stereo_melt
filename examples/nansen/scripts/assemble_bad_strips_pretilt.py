"""Pre-tilt-fit assembly of nansen per-DEM ``BAD_STRIPS`` (Shean-faithful).

Ported from :mod:`pig.scripts.assemble_bad_strips_pretilt`. The production
screen :mod:`nansen.scripts.find_bad_epochs` evaluates the bad-strip gates
on the *tilt-corrected* stack plus the tilt-params file, so it can only run
*after* :mod:`nansen.tilt_fit`. This script assembles the same drop list
...
Run::
    ...
    python -m nansen.scripts.assemble_bad_strips_pretilt --res 250
"""
