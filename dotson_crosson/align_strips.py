"""Batch ASP coregistration of downloaded REMA strips for DotsonCrosson.
...
Study-specific pieces live in ``dotson_crosson.config``.

Run:

    python -m dotson_crosson.align_strips                 # all unaligned strips
    python -m dotson_crosson.align_strips --limit 1       # one strip (smoke test)
    python -m dotson_crosson.align_strips --dem-id <id>   # specific strip by id
"""
...
def main() -> None:
    rc = run_align_strips_main(
        basin="dotson_crosson",
        aoi_shp=config.DC_AOI_SHP,
