...
| 7e | Space–time assimilation, B-PINN (diagnostic, separate JAX env) | `pig.prep_bpinn_trunk` → `pig.run_bpinn` | `dynamics.bpinn.fit_bpinn` | Thickness surrogate `H(x,y,t)` + melt network fitted to every finite pixel of every epoch with the thickness equation as a residual likelihood; the bridging transfer enters as an FFT observation operator; per-epoch datum/tilt nuisance planes; posterior by MAP → epoch-bootstrap ensemble → NUTS. Validated on the DEM-stack twin (transfer restores 2–3H channels: corr 0.23 → 0.70; `elmer_synth.prep_bpinn_twin` → `elmer_synth.run_bpinn_twin` at `--sigma-r 1 --col-slices 24`, and the twin tier is a local dataset, so packaging a twin only runs on the analysis host) and the PIG trunk window (2026-09-13). Two checks before believing a run: the fitted `time trend` must match the observed thinning, and `--no-transfer` must change the answer. **Not a melt product.** |
