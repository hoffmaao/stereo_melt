"""Pre-cache ICESat-1 GLAS (GLAH12) GCPs for the DotsonCrosson stack.

Thin wrapper over :mod:`stereo_melt.coregister.cache_glas`. Per-strip
filtered CSVs land under ``dotson_crosson/data/ASP/glas_data/``; the
NSIDC granule store is shared across basins at
``<MAIN_DIR>/data/GLAS/granules/``. GLAS reaches strips dated
≤ 2010-10-11 (last campaign + the Shean ±1 yr control cap). Window
override:

    DC_FETCH_START=2009-01-01 DC_FETCH_END=2010-10-12 \
        python -m dotson_crosson.cache_glas
"""
...
def main() -> None:
    ...
    rc = cache_glas.run_cache_glas_main(
        ...
        description="Cache ICESat-1 GLAS (GLAH12) GCPs for the DotsonCrosson stack.",
    )
    if rc:
        sys.exit(rc)
