"""Score Beardmore_Shelf epochs by post-tilt-fit static-control residual + IRLS weight.

Thin wrapper over :mod:`stereo_melt.coregister.tilt_qc`, at parity with
:mod:`pig.scripts.find_bad_epochs` (the Shean coregistration-quality screen,
2026-06-20 form). Run::

    python -m beardmore_shelf.scripts.find_bad_epochs            # 125 m production stack
    python -m beardmore_shelf.scripts.find_bad_epochs --res 25   # 25 m first-run variant

The ``--res`` switch mirrors :mod:`beardmore_shelf.tilt_fit` /
:mod:`beardmore_shelf.run_melt`: it selects the
``beardmore_shelf_stack_<N>m`` tilt-corrected stack and the matching
``beardmore_shelf_tilt_params_<N>m_*`` parameters, so the screen runs on
whichever stack the downstream solvers actually consume.
"""
...
def main(res_override: float | None = None, tag: str | None = None,
    ...
    print(f"  scoring {src_path.name}  ({tc.sizes['time']} epochs after BAD_STRIPS drop)")
    ...
    # P4 (pig parity): join per-strip pc_align quality (end_p50) so the screen
    # can drop geometry-failed strips the tilt-residual gate alone misses.
    ...
    if "dem_id" in suggested.columns:
        ...
        print("  ↳ per-DEM rejection (Shean): paste into config.BAD_STRIPS "
              "and EMPTY config.BAD_EPOCHS -- on a strip-addressable stack "
              "reject strips by dem_id, never by date (date-keying over-drops "
        ...
        print("  ⚠️ stack carries no dem_id coord -- this date-keyed list will "
              "also drop clean same-day siblings. Rebuild with "
              "`beardmore_shelf.build_stack` to enable per-strip BAD_STRIPS.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "beardmore_shelf_stack_<N>m_tilt_corrected_<start>_<end>.nc + "
            "beardmore_shelf_tilt_params_<N>m_<start>_<end>.nc."
        ),
    )
    parser.add_argument(
        "--tag",
        default=None,
        help=(
            "Experiment tag matching the `beardmore_shelf.build_stack --tag` "
            "build: loads beardmore_shelf_stack_<N>m_<tag>_tilt_corrected_*.nc "
            "+ beardmore_shelf_tilt_params_<N>m_<tag>_*.nc."
        ),
    )
    parser.add_argument(
        "--end-p50",
        ...
        help=(
            ...
            "10 m sits well above the clean IS2-era population (sub-metre) and "
            "above the 3 m offset-only demote band "
            "(BEARDMORE_SHELF_OFFSET_ONLY_END_P50_M), so only genuine pc_align "
            "divergence is dropped. Pass a large value to effectively disable; "
            "combine with the |med_resid|>50 m catastrophic backstop."
        ),
    )
