"""Pre-cache ICESat-2 ATL06 control points for every PIG strip.

Run once before ``pig.align_strips``. Each in-window strip gets a
...
Run:
    ...
    python -m pig.cache_icesat2                  # all in-window strips
    python -m pig.cache_icesat2 --overwrite      # refresh every strip
    python -m pig.cache_icesat2 --dem-id <id>    # single strip
    python -m pig.cache_icesat2 --limit 3        # smoke test first 3
"""
...
def _load_aoi() -> "shape":
    # Mirror cache_atm / cache_lvis / align_strips / fetch_strips: cache IS2
    # control across the wider stack extent when available so the upstream
    # band that build_stack reprojects has airborne control.
...
def main() -> None:
    ...
    print(f"\n=== IS2 cache summary: {counts['ok']} new, "
          ...
          f"(of {n} strips) ===")
    if n > 0 and (counts["ok"] + counts["skip"]) == 0:
        sys.exit(1)
