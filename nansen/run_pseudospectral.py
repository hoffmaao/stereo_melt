"""Apply both pseudo-spectral linear-perturbation inverses to Nansen.

Loads the existing Nansen stack + floating mask + velocity, runs the
...
def plot_pseudospectral_summary(
    ...
    fig.suptitle(
        f"Pseudo-spectral linear-perturbation inverses — Nansen "
...
def main(res_override: float | None = None) -> None:
    ...
    out_nc = (
        config.RESULTS_DIR
        / f"nansen_pseudospectral{out_suffix}_{config.START_TIME}_{config.END_TIME}.nc"
    )
    print(f"\nSaving -> {out_nc}")
    ds_out = xr.Dataset(
        {
    ...
    out_png = config.FIGURES_DIR / f"pseudospectral_summary{out_suffix}.png"
