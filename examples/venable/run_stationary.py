...
from venable.run_melt import load_floating_mask, load_velocity_on_grid
...
def main() -> None:
    ...
    if "tilt_corrected" in stack_path.name:
        raw_path = (
            config.PROCESSED_DIR
            / f"venable_stack_{config.START_TIME}_{config.END_TIME}.nc"
        )
        if not raw_path.exists():
            candidates = [
                p for p in sorted(config.PROCESSED_DIR.glob("venable_stack_*.nc"))
    ...
    # 4-year Venable stack, the right operating scale is well below
    ...
    out_nc = config.RESULTS_DIR / f"venable_stationary_{config.START_TIME}_{config.END_TIME}.nc"
