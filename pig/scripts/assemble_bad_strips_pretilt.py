"""Pre-tilt-fit assembly of PIG per-DEM ``BAD_STRIPS`` (Shean-faithful).

The production screen :mod:`pig.scripts.find_bad_epochs` evaluates the
bad-strip gates on the *tilt-corrected* stack plus the ``pig_tilt_params``
file, so it can only run *after* :mod:`pig.tilt_fit`. This script
assembles the same drop list *before* the first tilt fit, from the two
gates that need no tilt outputs, so ``config.BAD_STRIPS`` can be set and
the stack tilt-fit exactly ONCE:
    ...
    (b) ``|raw static-median| > 50 m`` -- catastrophic blunder backstop,
        ...
        per-pixel temporal median). This is the pre-tilt proxy for the
        post-tilt ``med_resid_m`` gate; it catches clean-``end_p50``
        blunders that gate (a) misses -- e.g. the old 2018-12-31 strip
        that aligns to ``end_p50 <= 2.4 m`` yet reads +123 m over control.

Gate (c) (IRLS hard-failure) is only observable post-tilt and is left to
the ``find_bad_epochs`` confirmation screen.

Gate evaluation REUSES the production :func:`suggest_bad_epochs` rather
than re-implementing the thresholds: we hand it a pre-tilt ``df`` with
``med_resid_m`` := the raw static-median deviation, ``weight_mean`` := a
finite sentinel, and ``drop_irls_failed=False``. So gates (a) and (b)
are computed by the exact code that will confirm them post-tilt.
...
def main(res: int = 250, tag: str = "is2ctempo") -> None:
    # Target the exact raw (non-tilt-corrected) stack name, mirroring how
    # load_basin_stack / find_bad_epochs name it. A loose glob is unsafe:
    # tagged siblings like ``..._is2ctempo_p3_...`` (a separate experiment,
    # no dem_id) share the ``is2ctempo`` prefix and sort *after* the plain
    # name, so glob-and-take-last grabs the wrong file.
    raw = config.PROCESSED_DIR / (
        f"pig_stack_{res}m_{tag}_{config.START_TIME}_{config.END_TIME}.nc"
    )
    if not raw.exists():
    ...
    if old_eps:
        ...
        for e in sorted(old_eps):
            ...
            tail = "" if n_flag else ("  <- now CLEAN under ctempo align "
                                      "(was flagged under the stale is2cs2 aq)")
            print(f"      {e}: {n_flag}/{n_day} strip(s) flagged, "
                  f"max end_p50={mx:.1f}m{tail}")

    # ---- paste-ready ----
    print("\n# ---- paste into pig/config.py ----")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--tag", default="is2ctempo")
    a = ap.parse_args()
    main(res=a.res, tag=a.tag)
