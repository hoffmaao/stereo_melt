...
spatially WHITE noise (0.3-0.9 m). Every twin-based solver conclusion is
calibrated against that model. This script measures the same quantities on a
real (or twin) stack so the two can be compared like-for-like:
...
jitter/coregistration ripple real strips carry and the twin does not).
...
def per_epoch_stats(stack: xr.DataArray, block_kms=(2.0, 4.0), min_px=500):
    ...
    for i in range(z.shape[0]):
        ...
        r2 = np.where(m, r - off - coef[0] * XX - coef[1] * YY - coef[2], np.nan)
        rms = float(np.sqrt(np.nanmean(r2 ** 2)))
        row = dict(offset_m=off, tilt_dm_km=10.0 * tilt_m_km, rms_m=rms,
                   px=int(m.sum()))
        for L in block_kms:
