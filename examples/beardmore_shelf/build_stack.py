"""Build the Beardmore_Shelf repeat-DEM stack on a common EPSG:3031 grid.

Stage 3 of the Beardmore_Shelf pipeline. Consumes ASP-aligned DEM GeoTIFFs
...
Run directly:
    ...
    python -m beardmore_shelf.build_stack
"""
...
def _stack_aoi_path() -> Path:
    ...
    return Path(config.BEARDMORE_SHELF_AOI_SHP)
...
def _qc_rejection_csv_for_variant(variant: str) -> Path | None:
    ...
    Returns the standard QC-list location used by ``beardmore_shelf.qc_strips``.
...
def plot_stack_footprints(
    ...
    ax.set_title(
        f"Beardmore_Shelf stack footprints\n"
...
def plot_stack_coverage(stack: xr.DataArray, output_path: Path) -> None:
    ...
    fig.suptitle(
        f"Beardmore_Shelf stack — {n} epochs on {stack.sizes['y']} x {stack.sizes['x']} grid",
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=140, bbox_inches="tight")
...
def main(
    ...
    tag: str | None = None,
    asp_suffix: str | None = None,
) -> xr.DataArray:
    config.ensure_output_dirs()

    # Variant suffix (feedback_res_suffix_convention): --res N and/or
    # --tag <t> write beardmore_shelf_stack[_<N>m][_<t>]_*.nc so the
    # config.RES production stack is never clobbered.
    if res_override is not None:
        target_res = float(res_override)
        out_suffix = f"_{int(round(target_res))}m"
    else:
    ...
    # Alignment-variant source override: --asp-suffix _is2ctempo reads
    # beardmore_shelf/data/ASP_is2ctempo/asp_aligned (an align_strips
    # --asp-suffix run) instead of config.STRIP_SOURCES.
    if asp_suffix:
        sources = [(
            config.BASIN_DIR / "data" / f"ASP{asp_suffix}" / "asp_aligned",
            tag or asp_suffix.lstrip("_"),
        )]
    else:
        sources = config.STRIP_SOURCES

    print("Listing ASP-aligned DEMs across fused sources...")
    for src_dir, variant in sources:
        print(f"  source [{variant}]: {src_dir}")
    candidates = list_aligned_dems(sources=sources)
    ...
    # clean grid lines. Also pad slightly so strips fully inside have edges.
    res = target_res
    ...
    out_nc = config.PROCESSED_DIR / (
        f"beardmore_shelf_stack{out_suffix}_{config.START_TIME}_{config.END_TIME}.nc"
    )
    print(f"Saving stack -> {out_nc}")
    save_stack(stack, out_nc)

    print("Writing QC figures to", config.FIGURES_DIR)
    plot_stack_footprints(kept, config.BEARDMORE_SHELF_AOI_SHP, config.FIGURES_DIR / "stack_footprints.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Grid resolution override (meters). When set, writes "
            "beardmore_shelf_stack_<N>m_<start>_<end>.nc so the config.RES "
            "production stack isn't clobbered."
        ),
    )
    parser.add_argument(
        "--tag",
        ...
        default=None,
        help=(
            "Variant tag appended to the stack name (after any _<N>m), "
            "e.g. --tag is2ctempo -> beardmore_shelf_stack_is2ctempo_*.nc."
        ),
    )
    parser.add_argument(
        "--asp-suffix",
        ...
        default=None,
        help=(
            "Read aligned DEMs from beardmore_shelf/data/ASP<suffix>/"
            "asp_aligned (an `align_strips --asp-suffix` run) instead of "
            "config.STRIP_SOURCES."
        ),
    )
    args = parser.parse_args()
    main(res_override=args.res, tag=args.tag, asp_suffix=args.asp_suffix)
