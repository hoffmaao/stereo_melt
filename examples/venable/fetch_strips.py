"""Search the REMA strip index for the Venable AOI + study window and download.

Stage 0 of the Venable pipeline. Uses ``pdemtools`` (via
``stereo_melt.io.rema``) to query the s2s041 strip index for strips that
overlap ``config.VENABLE_AOI_SHP`` between ``config.START_TIME`` and
``config.END_TIME``, then downloads any missing ones to
``config.STRIPS_DIR``.

Run:

    python -m venable.fetch_strips
"""
...
def main() -> None:
    ...
    start = os.environ.get("VENABLE_FETCH_START", config.START_TIME)
    end = os.environ.get("VENABLE_FETCH_END", config.END_TIME)
