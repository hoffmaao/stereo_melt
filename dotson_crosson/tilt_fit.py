"""Per-epoch residual-tilt correction for the Dotson + Crosson DEM stack.

Stage 3.5 of the Dotson + Crosson pipeline. Loads the raw stack produced by
:mod:`dotson_crosson.build_stack`, builds a Shean ``ndinterp.py``-style
...
The Dotson + Crosson stack covers a wider domain than ``DC_AOI_SHP``
(see ``DC_STACK_AOI_SHP``: ~30 km extension into the Queen
...
Run:
    ...
    python -m dotson_crosson.tilt_fit
"""
...
def _load_raw_stack() -> tuple[xr.DataArray, Path]:
    return load_basin_stack(
        config.PROCESSED_DIR,
        "dotson_crosson_stack",
        config.START_TIME,
        config.END_TIME,
...
def plot_tilt_params(params: xr.Dataset, control: xr.DataArray, out_path: Path) -> None:
    ...
    fig.suptitle(
        f"Dotson + Crosson tilt fit — {config.START_TIME} to {config.END_TIME} "
...
def main() -> None:
    ...
    stack, src_path = _load_raw_stack()
    ...
    # Dotson + Crosson sits at ~-75°S, north of DTU22's -79°S coverage limit, so
    ...
    #      (the Dotson + Crosson stack extent is built wide enough -- via
    #      DC_STACK_AOI_SHP -- to include ~47% grounded + ~7%
    ...
    # Dotson + Crosson combined AOI ~9000 km^2; Shean's 40 km PIG default
    # would disable x/y slope fitting on narrower epochs, collapsing the
    # LSQ to a constant-only fit and shoving cross-grid tilt into αz.
    # 10 km lets the LSQ engage slopes when the static control spans ≥10 km.
    #
    # Ez (per-epoch αz prior) scales with the ALIGNMENT control source
    # actually used by ASP (parsed from the asp_aligned/<dem_id>.sources.json
    # sidecar, with the ASP-root suffix as a fallback). High-precision
    # GCPs (IS2 / ATM / LVIS) → Ez=0.1 m (tightened 2026-05-05 from 0.3 m
    # after the Nansen dh/dt diagnosis showed +0.07 m/yr static-control
    # bias survived Ez=0.3); CS2-only → Ez=2.0 m. See feedback_ez_per_gcp_source.
    ...
    # each strip's control mix (is2/atm/lvis at 0.1 m; cs2/rock at 2.0 m).
    print(f"Per-epoch Ez (best-source breakdown): {dict(ez_summary)}")
    print("Fitting per-epoch residual tilts (Shean 2019 defaults)...")
    ...
    out_nc = (
        config.PROCESSED_DIR
        / f"dotson_crosson_stack_tilt_corrected_{config.START_TIME}_{config.END_TIME}.nc"
    )
    ...
    params_nc = (
        config.PROCESSED_DIR
        / f"dotson_crosson_tilt_params_{config.START_TIME}_{config.END_TIME}.nc"
    )
    print(f"Saving tilt parameters -> {params_nc}")
    params.to_netcdf(params_nc)

    fig_path = config.FIGURES_DIR / "tilt_fit_qc.png"
