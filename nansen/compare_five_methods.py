"""5-method melt-rate comparison on Nansen IS2 stack.
...
Run::
    ...
    python -m nansen.compare_five_methods
"""
...
def main(res_override: float | None = None) -> None:
    ...
    out_nc = (
        config.RESULTS_DIR
        / f"nansen_five_methods{out_suffix}_{config.START_TIME}_{config.END_TIME}.nc"
    )
    print(f"\nSaving -> {out_nc}")
    out_ds = xr.Dataset(
        {
    ...
    panels = [
        ...
        ("3. dh/dt FFT\n(per-pixel OLS, λ=10)",                     dhdt_fft, (-5, 5)),
        ("4. dh/dt DCT\n(per-pixel OLS, λ=10)",                     dhdt_dct, (-5, 5)),
        (f"5. D-masked CG DCT\n(λ=0.1, L=H_ref/2={L_cg:.0f}m)",     cg_dct, (-5, 5)),
    ...
    fig.suptitle(
        f"Nansen IS2 melt-rate comparison — 5 methods + Davison 2023 "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "nansen_stack_<N>m*_<start>_<end>.nc and writes outputs at *<N>m* "
