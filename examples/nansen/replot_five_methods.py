...
Reads ``nansen/results/nansen_five_methods_<window>.nc`` (produced by the retired
``nansen.compare_five_methods``) and builds a 2x3 figure with all six
melt-rate fields on a single ±5 m/yr RdBu scale, so panels are directly
comparable to the rest of the Beardmore figures.

Run::

    python -m nansen.replot_five_methods
"""
...
def main() -> None:
    ...
    fig.suptitle(
        f"Nansen melt-rate comparison — 5 methods  "
