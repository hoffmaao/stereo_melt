"""Pre-cache ICESat-2 ATL06 control points for every Beardmore strip.

Run once before ``beardmore.align_strips``. Each in-window strip gets a
filtered IS2 cache at::

    <STRIPS_DIR>/ASP/icesat2_data/icesat2_filtered_<dem_id>.h5

(``.csv`` legacy caches from before the HDF5 migration are still
honoured as cache hits.)
...
Run:
    ...
    python -m beardmore.cache_icesat2                  # all in-window strips
    python -m beardmore.cache_icesat2 --overwrite      # refresh every strip
    python -m beardmore.cache_icesat2 --dem-id <id>    # single strip
    python -m beardmore.cache_icesat2 --limit 3        # smoke test first 3
"""
...
def _strip_polygon(strip_path: Path):
    """Return the strip's data footprint (not just bbox) in EPSG:3031.

    Mirrors what ``stereo_melt.io.strip.load_strip`` does, but cheaper
    since we only need the polygon. We use the bbox here because the
    sliderule query region is intersected with the grounded-ice
    shapefile downstream — bbox is sufficient and avoids loading the
    full raster.
    """
    return _strip_bbox(strip_path)
...
def _load_aoi() -> "shape":
    ...
    # fetch_strips and align_strips use). Per-strip IS2 query in
    # cache_one passes the strip's own polygon as strip_boundary, so
    # the AOI here only filters which strips are *considered*, never
    # the per-shot footprint.
...
def cache_one(
    ...
    try:
        out = download_icesat2_data(
            strip_boundary=_strip_polygon(strip_path),
