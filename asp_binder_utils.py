...
def read_geodiff(csv_fn):
    ...
    resid_gdf = gpd.GeoDataFrame(resid_df, geometry=gpd.points_from_xy(resid_df['easting'], resid_df['northing'], crs='EPSG:3031'))
    return resid_gdf

def plot_alignment_maps_altimetry(reference_altimetry_df,src_dem,initial_elevation_difference_fn,
                        aligned_elevation_difference_fn,plot_crs,output_dir,provider=ctx.providers.Esri.WorldImagery,diff_clim=(-5,5)):
    ...
    reference_altimetry = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(reference_altimetry_df['easting'], reference_altimetry_df['northing']),
        crs="EPSG:3031"
        )
    ...
    print("Source DEM masked array (src_dem_ma):", src_dem_ma)
    print("Source DEM stats - min:", np.nanmin(src_dem_ma), "max:", np.nanmax(src_dem_ma), "mean:", np.nanmean(src_dem_ma))
