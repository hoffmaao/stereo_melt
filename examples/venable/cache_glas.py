"""Pre-cache ICESat-1 GLAS (GLAH12) GCPs for the Venable stack.

Thin wrapper over :mod:`stereo_melt.coregister.cache_glas`. Per-strip
filtered CSVs land under ``venable/data/ASP/glas_data/``; the NSIDC
...
control cap). Window override:
    ...
    VENABLE_FETCH_START=2009-01-01 VENABLE_FETCH_END=2010-10-12 \
        python -m venable.cache_glas
"""
...
def main() -> None:
    ...
    rc = cache_glas.run_cache_glas_main(
        ...
        fetch_window_env=("VENABLE_FETCH_START", "VENABLE_FETCH_END"),
        description="Cache ICESat-1 GLAS (GLAH12) GCPs for the Venable stack.",
    )
    if rc:
        sys.exit(rc)
