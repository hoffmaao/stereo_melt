"""Pre-cache ICESat-2 ATL06 control points for every Nansen strip.

Run once before ``nansen.align_strips``. Each in-window strip gets a
filtered IS2 cache at::

    <ASP_ROOT>/icesat2_data/icesat2_filtered_<dem_id>.h5

where ``ASP_ROOT`` is the per-basin ASP working tree
(``nansen/data/ASP/``). ``.csv`` legacy caches from before the HDF5
migration are still honoured as cache hits.
...
Run:
    ...
    python -m nansen.cache_icesat2                  # all in-window strips
    python -m nansen.cache_icesat2 --overwrite      # refresh every strip
    python -m nansen.cache_icesat2 --dem-id <id>    # single strip
    python -m nansen.cache_icesat2 --limit 3        # smoke test first 3
"""
...
def _load_aoi() -> "shape":
    # Use the wider analysis-grid AOI to match fetch_strips / align_strips.
