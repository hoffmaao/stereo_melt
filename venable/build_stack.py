"""Build the Venable repeat-DEM stack on a common EPSG:3031 grid.

Stage 3 of the Venable pipeline. Consumes ASP-aligned DEM GeoTIFFs
...
def _stack_aoi_path() -> Path:
    ...
    return Path(config.VENABLE_AOI_SHP)
...
def _qc_rejection_csv_for_variant(variant: str) -> Path | None:
    ...
    Returns the standard QC-list location used by ``venable.qc_strips``.
...
def plot_stack_coverage(stack: xr.DataArray, output_path: Path) -> None:
    ...
    fig.suptitle(
        f"Venable stack — {n} epochs on {stack.sizes['y']} x {stack.sizes['x']} grid",
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=140, bbox_inches="tight")
...
def main(res_override: float | None = None) -> xr.DataArray:
    ...
    # Resolution-pilot suffix (feedback_res_suffix_convention): a --res N
    # run writes venable_stack_<N>m_*.nc so the production stack at
    # config.RES is never clobbered.
    ...
    out_nc = (
        ...
        / f"venable_stack{out_suffix}_{config.START_TIME}_{config.END_TIME}.nc"
    )
    print(f"Saving stack -> {out_nc}")
    save_stack(stack, out_nc)

    print("Writing QC figures to", config.FIGURES_DIR)
    plot_stack_footprints(
        kept, config.VENABLE_AOI_SHP,
        config.FIGURES_DIR / f"stack_footprints{out_suffix}.png",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "venable_stack_<N>m_<start>_<end>.nc so the config.RES "
