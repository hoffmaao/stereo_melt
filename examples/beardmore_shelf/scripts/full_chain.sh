#!/usr/bin/env bash
# Beardmore_Shelf Stage A -- raw data to QC. Produces the tilt-corrected stack and the
...
#   2 cache_icesat2    -> beardmore_shelf/data/ASP/icesat2_data/* (IS2 ATL06 control)
#   3 align_strips     -> beardmore_shelf/data/ASP/asp_aligned/*  (ASP pc_align; long pole)
#   4 build_stack      -> processed/beardmore_shelf_stack_<window>.nc  (25 m EPSG:3031)
#   5 tilt_fit         -> processed/beardmore_shelf_stack_tilt_corrected_*.nc (first pass)
...
ASP_ROOT="$ROOT/beardmore_shelf/data/ASP"
PARALLEL="${BEARDMORE_SHELF_ALIGN_PARALLEL:-8}"
mkdir -p "$LOGDIR"
...
echo "[$(stamp)] === Beardmore_Shelf Stage A (data -> QC) start ==="
echo "[$(stamp)] /wd2 free: $(free_gb /wd2)  ASP_ROOT=$ASP_ROOT"

run_stage "1/6 fetch_strips"    fetch_strips.log    "$PY" -u -m beardmore_shelf.fetch_strips
run_stage "2/6 cache_icesat2"   cache_icesat2.log   "$PY" -u -m beardmore_shelf.cache_icesat2
...
run_stage "3/6 align_strips"    align_strips.log    "$PY" -u -m beardmore_shelf.align_strips --parallel "$PARALLEL"

run_stage "4/6 build_stack"     build_stack.log     "$PY" -u -m beardmore_shelf.build_stack
run_stage "5/6 tilt_fit"        tilt_fit.log        "$PY" -u -m beardmore_shelf.tilt_fit
run_stage "6/6 find_bad_epochs" find_bad_epochs.log "$PY" -u -m beardmore_shelf.scripts.find_bad_epochs

echo
echo "[$(stamp)] === Beardmore_Shelf Stage A done ==="
echo "NEXT (manual QC checkpoint):"
echo "  1. Review beardmore_shelf/logs/find_bad_epochs.log"
echo "  2. Paste the printed BAD_STRIPS tuple into beardmore_shelf/config.py"
echo "     (leave BAD_EPOCHS empty -- strip-level rejection is Shean-faithful)"
echo "  3. Launch the solvers:"
echo "       nohup setsid bash beardmore_shelf/scripts/run_solvers.sh \\"
echo "         > beardmore_shelf/logs/run_solvers.log 2>&1 < /dev/null &"
