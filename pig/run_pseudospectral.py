"""Apply both pseudo-spectral linear-perturbation inverses to PineIsland.

Loads the existing PIG stack + floating mask + velocity, runs the
...
Run:
    ...
    python -m pig.run_pseudospectral
"""
...
def plot_pseudospectral_summary(
    ...
    fig.suptitle(
        f"Pseudo-spectral linear-perturbation inverses — Pine Island "
...
def main() -> None:
    ...
    out_nc = (
        config.RESULTS_DIR
        / f"pig_pseudospectral_{config.START_TIME}_{config.END_TIME}.nc"
    )
    print(f"\nSaving -> {out_nc}")
    ds_out = xr.Dataset(
        {
