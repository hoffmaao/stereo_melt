"""Per-epoch residual-tilt correction for the McMurdo DEM stack.

Stage 3.5 of the McMurdo pipeline. Loads the raw stack produced by
:mod:`mcmurdo.build_stack`, builds a Shean ``ndinterp.py``-style
...
defaults (Shean 2019 McMurdo priors), and writes a tilt-corrected stack
as a new NetCDF that downstream melt-rate solvers consume directly.

The McMurdo stack covers a wider domain than ``MCMURDO_AOI_SHP``
(see ``MCMURDO_STACK_AOI_SHP``: ~30 km extension into the Queen
...
def plot_tilt_params(params: xr.Dataset, control: xr.DataArray, out_path: Path) -> None:
    ...
    fig.suptitle(
        f"McMurdo tilt fit — {config.START_TIME} to {config.END_TIME} "
...
def main() -> None:
    ...
    # McMurdo sits at ~-75°S, north of DTU22's -79°S coverage limit, so
    ...
    #      (the McMurdo stack extent is built wide enough -- via
    #      MCMURDO_STACK_AOI_SHP -- to include ~47% grounded + ~7%
    ...
    # McMurdo stack is ~23 km × 54 km; Shean's 40 km McMurdo default would
    ...
    out_nc = (
        config.PROCESSED_DIR
        / f"mcmurdo_stack_tilt_corrected_{config.START_TIME}_{config.END_TIME}.nc"
    )
    ...
    params_nc = (
        config.PROCESSED_DIR
        / f"mcmurdo_tilt_params_{config.START_TIME}_{config.END_TIME}.nc"
    )
