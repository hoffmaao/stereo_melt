"""Per-epoch residual-tilt correction for the Beardmore DEM stack.

Stage 3.5 of the Beardmore pipeline. Loads the raw stack produced by
:mod:`beardmore.build_stack`, builds a Shean ``ndinterp.py``-style
...
The Beardmore stack covers a wider domain than ``BEARDMORE_AOI_SHP``
(see ``BEARDMORE_STACK_AOI_SHP``: ~30 km extension into the Queen
...
def _load_raw_stack(stack_prefix: str = "beardmore_stack") -> tuple[xr.DataArray, Path]:
...
def plot_tilt_params(params: xr.Dataset, control: xr.DataArray, out_path: Path) -> None:
    ...
    fig.suptitle(
        f"Beardmore tilt fit — {config.START_TIME} to {config.END_TIME} "
...
def main(res_override: float | None = None) -> None:
    ...
    # MDT is skipped on Beardmore (lat ~-84°S, poleward of DTU22's
    # -79°S coverage limit), so the chain reduces to:
    #   tide -> IBE -> geoid -> tilt fit
    # All static-field corrections run BEFORE the per-epoch tilt LSQ so
    # the per-pixel intercept block carries residual elevation around
    # the orthometric surface, leaving alpha_z to absorb per-strip
    # coregistration drift only.
    ...
    #      (the Beardmore stack extent is built wide enough -- via
    #      BEARDMORE_STACK_AOI_SHP -- to include ~47% grounded + ~7%
    ...
    static_qc_path = config.FIGURES_DIR / f"static_mask_strip_count{out_suffix}.png"
    plot_static_mask_and_strip_count(
        stack, control.values, static_qc_path, title_prefix="Beardmore",
        aoi_path=config.BEARDMORE_AOI_SHP,
    )
    print(f"  wrote {static_qc_path}")

    # 3/3: per-epoch tilt LSQ on the geoid-corrected stack.
    # Beardmore stack is ~23 km × 54 km; Shean's 40 km PIG default would
    ...
    # (or inferred from the ASP-root suffix). Beardmore is the only basin
    # that crosses the IS2/CS2 era boundary, so its STRIP_SOURCES list two
    # roots: ASP_is2cs2 (post-Oct 2018, mixed IS2+CS2 control) and ASP_cs2
    # (pre-Oct 2018, CS2-only). build_per_epoch_ez walks both and returns
    # Ez=0.1 m for IS2-controlled epochs, Ez=2.0 m for CS2-only epochs.
    # See feedback_ez_per_gcp_source.
    ...
    params, stack_corr = fit_tilt_stack(
        stack, control_mask=control, min_width=10000, Ez=Ez_per_epoch
    )
    ...
    params_nc = (
        config.PROCESSED_DIR
        / f"beardmore_tilt_params{out_suffix}_{config.START_TIME}_{config.END_TIME}.nc"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "beardmore_stack_<N>m_<start>_<end>.nc built by `beardmore.build_stack --res <N>`. "
