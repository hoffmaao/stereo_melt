"""Run the McMurdo Lagrangian melt-rate pipeline on the built stack.

Stage 4+ of the McMurdo pipeline. Loads the already-built DEM stack,
...
returned Dataset plus QC plots to ``mcmurdo/figures/``.

Run:

    python -m mcmurdo.run_melt

Assumes :mod:`mcmurdo.build_stack` has already produced
``processed/mcmurdo_stack_<start>_<end>.nc``.
"""
...
def load_velocity_on_grid(stack: xr.DataArray) -> tuple[xr.DataArray, xr.DataArray, str]:
    ...
    Velocity source is selected by the ``MCMURDO_VELOCITY`` env var:
    ...
    """
    requested = os.environ.get("MCMURDO_VELOCITY", "measures").strip().lower()
    source = None
...
def plot_inputs(stack, vx, vy, a_dot, firn, out_path: Path) -> None:
    ...
    fig.suptitle("McMurdo melt-rate inputs", fontsize=12)
...
def plot_melt_comparison(
    ...
    fig.suptitle(
        f"McMurdo melt rate — {config.START_TIME} to {config.END_TIME}",
        fontsize=12,
    )
    fig.savefig(out_path, dpi=140, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    # reg=1e-1 from a sweep on the real McMurdo stack: lower reg leaves
    ...
    out_nc = (
        config.RESULTS_DIR
        / f"mcmurdo_melt_{config.START_TIME}_{config.END_TIME}.nc"
    )
