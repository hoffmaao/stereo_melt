"""Pre-tilt-fit assembly of dotson_crosson per-DEM ``BAD_STRIPS`` (Shean-faithful).

Ported from :mod:`pig.scripts.assemble_bad_strips_pretilt`. The production
screen :mod:`dotson_crosson.scripts.find_bad_epochs` evaluates the bad-strip gates
on the *tilt-corrected* stack plus the tilt-params file, so it can only run
*after* :mod:`dotson_crosson.tilt_fit`. This script assembles the same drop list
...
Run::
    ...
    python -m dotson_crosson.scripts.assemble_bad_strips_pretilt --res 250
"""
