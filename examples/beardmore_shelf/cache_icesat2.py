"""Pre-cache ICESat-2 ATL06 control points for every Beardmore_Shelf strip.

Run once before ``beardmore_shelf.align_strips``. Each in-window strip gets a
filtered IS2 cache at::

    <ASP_ROOT>/icesat2_data/icesat2_filtered_<dem_id>.h5

where ``ASP_ROOT`` is the per-basin ASP working tree
(``beardmore_shelf/data/ASP/``). ``.csv`` legacy caches from before the
...
Run:
    ...
    python -m beardmore_shelf.cache_icesat2                  # all in-window strips
    python -m beardmore_shelf.cache_icesat2 --overwrite      # refresh every strip
    python -m beardmore_shelf.cache_icesat2 --dem-id <id>    # single strip
    python -m beardmore_shelf.cache_icesat2 --limit 3        # smoke test first 3
"""
...
def main() -> None:
    ...
    parser.add_argument("--max-fail-frac", type=float, default=0.25,
                        help="Exit non-zero only if the failed fraction exceeds "
                             "this (default 0.25, i.e. 25%% of in-window strips). "
                             "A few transient SlideRule glitches or genuinely "
                             "no-IS2 strips shouldn't kill an unattended chain; a "
                             "systemic outage (most strips failing) should.")
    ...
    print(f"\n=== IS2 cache summary: {counts['ok']} new, "
          ...
          f"(of {n} strips) ===")
    if counts["fail"]:
        # 'skip' = already-cached (healthy control); only 'fail' lacks IS2. Gauge
        # systemic health by the fraction of the whole in-window set that ends up
        # without control, so a tiny delta-fetch with one bad strip can't trip it.
        fail_frac = counts["fail"] / n if n else 0.0
        print(f"  {counts['fail']} strip(s) lack IS2 control "
              f"({fail_frac:.1%} of {n}; abort threshold {args.max_fail_frac:.0%}).")
        if fail_frac > args.max_fail_frac:
            print("  Fraction over threshold — likely a systemic SlideRule/CMR "
                  "outage; aborting so the long align doesn't run on missing "
                  "control.")
            sys.exit(1)
        print("  Below threshold — transient/no-data; treating as non-fatal so "
              "the chain continues (those strips just align without IS2 GCPs).")
