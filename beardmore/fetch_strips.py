"""Search the REMA strip index for the Beardmore AOI + study window and download.

Stage 0 of the Beardmore pipeline. Uses ``pdemtools`` (via
``stereo_melt.io.rema``) to query the s2s041 strip index for strips that
overlap ``config.BEARDMORE_AOI_SHP`` between ``config.START_TIME`` and
``config.END_TIME``, then downloads any missing ones to
``config.STRIPS_DIR``.

Run:

    python -m beardmore.fetch_strips
"""
...
def main() -> None:
    ...
    # than the basin's primary processing config (e.g. pre-IS2 strip
    # backfill that will only be coregistered once CS2 GCPs are validated).
    start = os.environ.get("BEARDMORE_FETCH_START", config.START_TIME)
    end = os.environ.get("BEARDMORE_FETCH_END", config.END_TIME)

    # Use the wider stack/tilt extent for fetching when defined — otherwise
    # we'd miss the 8.5× more strips needed to populate the wider analysis grid.
    aoi_path = config.BEARDMORE_AOI_SHP
    with fiona.open(aoi_path) as src:
        polygon = shape(next(iter(src))["geometry"])

    print(f"AOI: {config.SHELF} ({Path(aoi_path).name}), bounds={polygon.bounds}, area={polygon.area / 1e6:.1f} km^2")
