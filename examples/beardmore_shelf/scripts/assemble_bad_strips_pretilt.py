"""Pre-tilt-fit assembly of beardmore_shelf per-DEM ``BAD_STRIPS`` (Shean-faithful).

Ported from :mod:`pig.scripts.assemble_bad_strips_pretilt`. The production
screen :mod:`beardmore_shelf.scripts.find_bad_epochs` evaluates the bad-strip gates
on the *tilt-corrected* stack plus the tilt-params file, so it can only run
*after* :mod:`beardmore_shelf.tilt_fit`. This script assembles the same drop list
...
Run::
    ...
    python -m beardmore_shelf.scripts.assemble_bad_strips_pretilt --res 250
"""
