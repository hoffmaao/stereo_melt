...
| 3b | Nocorr ingestion (Shean) | `<basin>.ingest_nocorr` (beardmore_shelf; PIG since 2026-09, opt-in `PIG_SOURCES=nocorr`) | copy + class-bias shift | Strips no control can anchor (fully-floating / align-failed / no-cache / QC-readmitted) enter at a-priori geolocation + one **per-era class-mean z-offset** into `ASP_nocorr/`, to be datum-corrected by the tilt LSQ (Ez 1.0). Shean `stack_nocorr_adjust.py` analog. |
...
| 5e | Per-epoch tilt LSQ | inline | `coregister.tilt.fit_tilt_stack` | Joint LSQ: per-pixel intercept + trend + per-strip `(αx, αy, αz)`, Tikhonov priors, Tukey IRLS. **Per-epoch Ez** from `coregister.control_source.build_per_epoch_ez` resolved by `dem_id` against the align roots' sources sidecars (IS2/ATM/LVIS 0.1 m · GLAS 0.3 m · CryoTEMPO 1.0 m · nocorr 1.0 m · raw-CS2 2.0 m) — beardmore_shelf always; PIG only when a nocorr root is active or `PIG_TILT_EZ_BY_DEM_ID=1`, otherwise by date union so the canon products are unchanged. Two domains: `static` (control-only, legacy default) or `full` (Shean ndinterp parity: all ice pixels incl. shelf) — **`full` must pair with the dh/dt Laplacian smoothness rows** (`dhdt_smoothness=1.0`, Shean L574+), which is what lets cross-epoch self-consistency set datums for control-free epochs without the front-accretion aliasing nullspace (validated 2026-07-11 on beardmore_shelf; on PIG via `PIG_TILT_DOMAIN=full` + `PIG_TILT_DHDT_SMOOTH=1.0`, 2026-09, default static). Poorly-coregistered strips (pc_align `end_p50` > 3 m) are demoted to αz-only, not dropped — keyed by date by default (as the canon products were built) and per `dem_id` when a nocorr root is active or `PIG_TILT_EZ_BY_DEM_ID=1`, so a control-free layer is never demoted on a same-date strip's alignment. The slope priors `Ex`/`Ey` are per-basin (PIG: `config.TILT_EX/TILT_EY`, calibrated 2026-09-02 from the control residual planes of stage 6c — note the measured anisotropy is the *opposite* of Shean's `Ey = Ex/3`). `min_width` (the spatial-spread gate below which an epoch fits αz only) defaults to **0** in the library; all seven basin drivers pass their own (10 km at PIG) — which is why the default change moves no published number — and at that value it still forces αz-only on ~half the epochs. The only call sites that take the new default are `scripts/test_gpu_tilt_fit.py` (assertion-free GPU smoke script) and `tests/gate_tilt_stack.py`, whose 10 km synthetic has `dist_ptp` ~4.7 km and which asserts on the recovered slopes at that default. Why the previous default was retired: see the `min_width` docstring in `coregister/tilt.py`. |
...
| 6c | QC: control residual planes | `coregister.alignment_quality.residual_planes_for_strips` | same | Per strip, a robust plane through the **signed** residual of the aligned DEM against the control `pc_align` was fed (`reference_files/combined_reference_*.csv` — *not* `-end_errors.csv`, whose column is unsigned and pre-transform). Catches alignment failures that `end_p50` misses (partially-broken footprints pass a median screen; p84 and this scatter do not) — 15 such strips on the PIG canon, none of which the stage-6 screens had flagged; they are now in PIG's `config.BAD_STRIPS`. The same table supplies the coloured-noise prior via `dynamics.budget_bridging.strip_prior_from_residual_planes`, whose summary splits the excluded strips by REASON — `qc_alignment_failures` (`sd > max_sd`), `qc_low_control` (`n < min_n`), `qc_unfitted` (no plane at all, normally a missing control file); see that function's docstring for the full taxonomy. Only the alignment failures are `BAD_STRIPS` candidates: on PIG those were 15, and the 15 low-control strips were deliberately kept. |
...
| 7e | Space–time assimilation, B-PINN (diagnostic, separate JAX env) | `pig.prep_bpinn_trunk` → `pig.run_bpinn` | `dynamics.bpinn.fit_bpinn` | Thickness surrogate `H(x,y,t)` + melt network fitted to every finite pixel of every epoch with the thickness equation as a residual likelihood; the bridging transfer enters as an FFT observation operator; per-epoch datum/tilt nuisance planes; posterior by MAP → epoch-bootstrap ensemble → NUTS. Validated on the noise-free DEM-stack twin (transfer restores 2–3H channels: corr 0.231 → 0.693, re-measured 2026-09-13 on the current packaging after every orientation fix). On the PIG trunk window the drivers were exercised (2026-09-13) and the fit passes the trend and ablation checks, but every recorded PIG result predates the transfer-orientation fixes and is a diagnostic only: repeat the trunk runs with the corrected operator before quoting any number from them. To reproduce the twin number, package the noise-free tier with `prep_bpinn_twin.py multixy_pigreal multixy_bmb clean` (writes `twin_multixy_pigreal_clean.npz` into `<checkout>/examples/elmer_synth/results/bpinn`, the first directory `run_bpinn_twin.py` searches, so packaging and scoring always name the same twin) and score it with `run_bpinn_twin.py --tag multixy_pigreal_clean --transfer --sigma-h 0.5 --no-planes --sigma-r 1 --col-slices 24 --steps 10000 --ensemble 1`: corr 0.231 without `--transfer`, 0.693 with it (nrmse 0.995 → 0.764), on the single-member `B-PINN MAP` row, measured 2026-09-13 on the packaging in this tree (the production Eulerian and Lagrangian rows of the same run are corr 0.206 and 0.120). The default tilt-corrected, error-injected tier is noise-limited there (corr about 0). The twin tier is a local dataset, so packaging a twin only runs on the analysis host. Two checks before believing a run: the fitted `time trend` must match the observed thinning, and `--no-transfer` must change the answer. Where one mean geometry does not represent the window (thickness, speed or flow direction varying across it), `--n-bins N --blend-px P` makes the operator local: bins on (H, u_x, u_y) with partition-of-unity blending, the monolithic solver's `n_bins` construction (`tests/gate_bpinn_local_transfer.py`); default 1, because localisation measured neutral-to-slightly-worse everywhere it has been run — noise-free twin corr 0.69 (1 bin) vs 0.67 (4–8 bins), and a half-stack signal-to-noise tie on the PIG trunk (σ 21.7 vs 22.9 m/yr at 6 bins) whose change reproduces across halves only in the 800–1300 m band (measured 2026-09-13) — so it is opt-in and **not a claimed improvement**. **Not a melt product.** |
...
- **Area-mean error bars need `n_eff`, not the pixel count**
  ...
  *not* identifiable from the domain (the fitted range tracks `max_lag`), so
  use the model-free block estimator: 3–5 Gt/yr, against 0.7 Gt/yr if the
  noise were white.
