"""Pre-cache ICESat-2 ATL06 control points for every Venable strip.

Run once before ``venable.align_strips``. Each in-window strip gets a
filtered IS2 cache at::

    <ASP_ROOT>/icesat2_data/icesat2_filtered_<dem_id>.h5

where ``ASP_ROOT`` is the per-basin ASP working tree
(``venable/data/ASP/``). ``.csv`` legacy caches from before the
...
Run:
    ...
    python -m venable.cache_icesat2                  # all in-window strips
    python -m venable.cache_icesat2 --overwrite      # refresh every strip
    python -m venable.cache_icesat2 --dem-id <id>    # single strip
    python -m venable.cache_icesat2 --limit 3        # smoke test first 3
"""
