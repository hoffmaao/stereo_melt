...
# outer cap) for the 29 PIG strips that hit
...
echo "[$(stamp)] === PIG Shean re-cache for $N pc_align-failed strips ==="
...
while IFS= read -r dem_id; do
    ...
    if $PY -u -m pig.cache_icesat2 --overwrite --dem-id "$dem_id"; then
        ok=$((ok+1))
...
echo
echo "[$(stamp)] === PIG Shean re-cache done: $ok ok, $fail fail (of $N) ==="
