"""Pre-tilt-fit assembly of venable per-DEM ``BAD_STRIPS`` (Shean-faithful).

Ported from :mod:`pig.scripts.assemble_bad_strips_pretilt`. The production
screen :mod:`venable.scripts.find_bad_epochs` evaluates the bad-strip gates
on the *tilt-corrected* stack plus the tilt-params file, so it can only run
*after* :mod:`venable.tilt_fit`. This script assembles the same drop list
...
Run::
    ...
    python -m venable.scripts.assemble_bad_strips_pretilt --res 250
"""
