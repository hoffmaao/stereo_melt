"""RACMO2.4p1 SMB loader and regridder.
...
RACMO_MASK_URL = (
    "https://zenodo.org/api/records/19255213/files/ANT11_masks.nc/content"
)
...
def load_racmo_smb(smb_path):
    ...
    if "smbgl" not in ds.variables:
        raise KeyError(
            f"Expected variable 'smbgl' in {smb_path}; found {list(ds.data_vars)}"
        )
    smb = ds["smbgl"]
...
def regrid_smb_to_xy(smb_window, target_x, target_y, method="linear"):
    ...
    if lat.ndim == 1 and lon.ndim == 1:
        ...
        raise ValueError(
            f"Unexpected lat/lon dims: lat.ndim={lat.ndim}, lon.ndim={lon.ndim}"
        )
