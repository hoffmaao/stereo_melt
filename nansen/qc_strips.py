"""Hard-reject DEM strips with poor pc_align residuals (Nansen).

Mirror of :mod:`beardmore.qc_strips` for the Nansen single-variant
``is2`` alignment run.

Reads ``results/geodiff_comparison_per_strip.csv`` (the per-variant
post-pc_align residual table emitted by ``nansen.compare_geodiff``) and
...
Run::
    ...
    python -m nansen.qc_strips
    python -m nansen.qc_strips --min-n 30 --max-rmse 1.5

Then re-run ``nansen.build_stack``; it auto-discovers the rejection CSV
and skips matching strips.
"""
...
from nansen.compare_geodiff import VARIANTS

DEFAULT_GEODIFF_CSV = config.RESULTS_DIR / "geodiff_comparison_per_strip.csv"
DEFAULT_OUT_DIR = config.RESULTS_DIR
