"""Pre-cache IceBridge LVIS (ILVIS2) GCPs for the Nansen Ice Shelf stack.
...
root (``nansen/data/ASP/lvis_data/``); the upstream NSIDC granule store is
shared across all basins under ``<MAIN_DIR>/data/LVIS/granules/``.

Run:

    python -m nansen.cache_lvis                        # all in-window strips
    python -m nansen.cache_lvis --prefetch-only        # warm shared granule store
    python -m nansen.cache_lvis --prefetch-only --prefetch-dry-run  # CMR query only
...
def main() -> None:
    ...
    rc = cache_airborne.run_cache_airborne_main(
        ...
        description="Cache IceBridge LVIS (ILVIS2) GCPs for the Nansen stack.",
    )
    if rc:
        sys.exit(rc)
