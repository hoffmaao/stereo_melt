"""Standard QC plot: Beardmore wider-AOI LS mask + per-pixel strip count.
...
This is also called from ``beardmore.tilt_fit`` so the figure is part
of the standard tilt-fit run; this script is a standalone re-render
against the persisted tilt-corrected stack.

Run:

    /home/hoffmaao/miniconda3/envs/stereo_melt/bin/python -m beardmore.plot_static_mask
"""
...
def _pick_stack() -> Path:
    """Prefer the wider-AOI tilt-corrected stack; fall back to raw."""
    candidates = [
        config.PROCESSED_DIR / f"beardmore_stack_tilt_corrected_{config.START_TIME}_{config.END_TIME}.nc",
        config.PROCESSED_DIR / f"beardmore_stack_{config.START_TIME}_{config.END_TIME}.nc",
    ]
    for p in candidates:
        if p.exists():
            return p
...
def main() -> None:
    ...
    plot_static_mask_and_strip_count(
        stack, control, out_path, title_prefix="Beardmore",
