"""Stationary CG-LSQ pseudospectral inverse driver for Beardmore_Shelf.

Calls :func:`stereo_melt.dynamics.stationary_pseudospectral_lagrangian_inverse`
on the existing Beardmore_Shelf stack. Sweeps Tikhonov regularization so the
operator point at the right magnitude can be picked from the recovered
medians and the residual norm.

Run:

    python -m beardmore_shelf.run_stationary
"""
...
from beardmore_shelf.run_melt import load_floating_mask, load_velocity_on_grid
...
def plot_tikhonov_sweep(results: list[tuple[float, xr.Dataset]], out_path: Path) -> None:
    ...
    for k in range(n, nrows * ncols):
        ...
        axes[r, c].axis("off")
    fig.suptitle(
        f"Stationary CG-LSQ — Beardmore_Shelf "
...
def main() -> None:
    ...
    if "tilt_corrected" in stack_path.name:
        raw_path = (
            config.PROCESSED_DIR
            / f"beardmore_shelf_stack_{config.START_TIME}_{config.END_TIME}.nc"
        )
        if not raw_path.exists():
            candidates = [
                p for p in sorted(config.PROCESSED_DIR.glob("beardmore_shelf_stack_*.nc"))
    ...
    # 4-year Beardmore_Shelf stack, the right operating scale is well below
    ...
    out_nc = config.RESULTS_DIR / f"beardmore_shelf_stationary_{config.START_TIME}_{config.END_TIME}.nc"
