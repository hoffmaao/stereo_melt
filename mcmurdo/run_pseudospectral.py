"""Apply both pseudo-spectral linear-perturbation inverses to McMurdo.

Loads the existing McMurdo stack + floating mask + velocity, runs the
...
def plot_pseudospectral_summary(
    ...
    fig.suptitle(
        f"Pseudo-spectral linear-perturbation inverses — McMurdo "
...
def main() -> None:
    ...
    out_nc = (
        config.RESULTS_DIR
        / f"mcmurdo_pseudospectral_{config.START_TIME}_{config.END_TIME}.nc"
    )
    print(f"\nSaving -> {out_nc}")
    ds_out = xr.Dataset(
        {
