#!/usr/bin/env bash
# Re-run pc_align on the 15 Nansen strips that previously failed with
...
echo "[$(stamp)] === Nansen re-align for $N pc_align-failed strips (Shean caches) ==="
...
while IFS= read -r dem_id; do
    ...
    if $PY -u -m nansen.align_strips --dem-id "$dem_id"; then
        ok=$((ok+1))
...
echo
echo "[$(stamp)] === Nansen re-align done: $ok ok, $fail fail (of $N) ==="
