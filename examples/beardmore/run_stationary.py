...
from beardmore.run_melt import load_floating_mask, load_velocity_on_grid
...
def main() -> None:
    ...
    if "tilt_corrected" in stack_path.name:
        ...
        if not raw_path.exists():
            candidates = [
                p for p in sorted(config.PROCESSED_DIR.glob("beardmore_stack_*.nc"))
    ...
    # 4-year Beardmore stack, the right operating scale is well below
    ...
    out_nc = config.RESULTS_DIR / f"beardmore_stationary_{config.START_TIME}_{config.END_TIME}.nc"
