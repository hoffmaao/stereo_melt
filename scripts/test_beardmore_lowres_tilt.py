"""Low-resolution Beardmore tilt_fit test on GPU.

Uses the just-built Beardmore wider-AOI stack, coarsens to a tiny grid,
and runs fit_tilt_stack with cupy. Verifies the GPU LSMR + IRLS path
end-to-end on real Beardmore data without waiting for the full-res run.

Usage:
    CUDA_VISIBLE_DEVICES=0 STEREO_MELT_BACKEND=cupy \\
      python scripts/test_beardmore_lowres_tilt.py [coarsen_factor]
"""
...
STACK_NC = EXAMPLES / "beardmore/processed/beardmore_stack_2013-01-01_2023-03-01.nc"
BEDMACHINE = ROOT / "data/bedmachine/BedMachineAntarctica-v3.nc"


def main(coarsen: int = 32) -> None:
    print(f"backend: STEREO_MELT_BACKEND={os.environ.get('STEREO_MELT_BACKEND', '(unset)')}")
    print(f"CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES', '(unset)')}")

    print(f"\nLoading Beardmore wider-AOI stack: {STACK_NC.name}")
    ...
    params, stack_corr = fit_tilt_stack(
        coarse,
        control_mask=static,
        Eint=10.0, Edhdt=10.0, Ex=1e-3, Ey=1e-3, Ez=0.5,
    ...
    print(f"  params dataset variables: {list(params.data_vars)}")
    if "tilt_dx" in params.data_vars:
        print(f"  tilt_dx range: [{float(params['tilt_dx'].min()):+.2e}, {float(params['tilt_dx'].max()):+.2e}] /m")
        print(f"  tilt_dy range: [{float(params['tilt_dy'].min()):+.2e}, {float(params['tilt_dy'].max()):+.2e}] /m")
        print(f"  tilt_dz range: [{float(params['tilt_dz'].min()):+.3f}, {float(params['tilt_dz'].max()):+.3f}] m")
    print(f"\nSUCCESS — GPU LSMR + IRLS path is working at coarsen={coarsen}× on Beardmore data.")


if __name__ == "__main__":
    coarsen = int(sys.argv[1]) if len(sys.argv) > 1 else 32
