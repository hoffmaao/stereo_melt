"""Build the Beardmore repeat-DEM stack on a common EPSG:3031 grid.

Stage 3 of the Beardmore pipeline. Consumes ASP-aligned DEM GeoTIFFs
...
Run directly:
    ...
    python -m beardmore.build_stack
"""
...
def _stack_aoi_path() -> Path:
    ...
    return Path(config.BEARDMORE_AOI_SHP)
...
def _aoi_elevation_envelope(
    ...
    envelope is also ellipsoid-referenced.

    The wider Beardmore stack AOI spans ~0 m (floating tongue) to
    ~3500 m (Queen Alexandra Range), so the hard-coded ``[-200, 1500]``
    envelope of the previous narrow-AOI build dropped any strip
    catching real mountain pixels. Computing the envelope from
    BedMachine per-AOI makes the threshold travel with the AOI.
    """
    import xarray as xr
...
def _alignment_sanity_check(
    ...
    medians stay within the elevation envelope of the surfaces in the
    AOI. The caller should supply ``[median_min, median_max]`` as
    ``z_min - pad`` / ``z_max + pad`` for ``z_min, z_max`` from
    BedMachine surface in the AOI (see :func:`_aoi_elevation_envelope`)
    so the envelope tracks the AOI rather than hard-coding a Beardmore-
    specific range.
...
def _qc_rejection_csv_for_variant(variant: str) -> Path | None:
    ...
    Returns the standard QC-list location used by ``beardmore.qc_strips``.
...
def plot_stack_coverage(stack: xr.DataArray, output_path: Path) -> None:
    ...
    fig.suptitle(
        f"Beardmore stack — {n} epochs on {stack.sizes['y']} x {stack.sizes['x']} grid",
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=140, bbox_inches="tight")
...
def main(res_override: float | None = None) -> xr.DataArray:
    ...
    # had disabled when the wider Beardmore AOI started sampling Queen
    # Alexandra Range rock (~3500 m), which the old hard-coded
    # [-200, +1500] envelope rejected wholesale. The dynamic envelope
    # still catches pc_align catastrophic shifts (those move medians by
    # 1-10 km, way outside any plausible AOI elevation envelope).
    ...
    print(f"AOI elevation envelope (BedMachine surface ± 100 m): "
          f"[{zmin:+.0f}, {zmax:+.0f}] m")
    print("Filtering by alignment sanity check (median in envelope)...")
    kept, rejected = _alignment_sanity_check(
        kept, aoi_bb, median_min=zmin, median_max=zmax,
    )
    print(f"  kept {len(kept)} after sanity check (rejected {len(rejected)})")
    for p, t, v, med in rejected:
        print(f"    sanity-rejected: {t.date()}  [{v}]  median={med:+.1f} m  {p.name}")
    ...
    # order as source_variant. Keystone for *strip-level* bad-epoch dropping:
    # REMA filenames carry no time-of-day, so _extract_date collapses many
    # same-day strips to one timestamp; a date-keyed drop then discards clean
    # same-day siblings along with the one bad strip. dem_id restores Shean
    # 2019's per-DEM rejection granularity (his DEMs were sub-minute-unique, so
    # date==DEM for him; ours are not). Consumed by load_basin_stack(bad_strips=).
    dem_ids = np.asarray(
    ...
    out_nc = config.PROCESSED_DIR / f"beardmore_stack{out_suffix}_{config.START_TIME}_{config.END_TIME}.nc"
    print(f"Saving stack -> {out_nc}")
    save_stack(stack, out_nc)

    print("Writing QC figures to", config.FIGURES_DIR)
    plot_stack_footprints(kept, config.BEARDMORE_AOI_SHP, config.FIGURES_DIR / f"stack_footprints{out_suffix}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "is built on an <N>-m grid and saved to beardmore_stack_<N>m_*.nc "
