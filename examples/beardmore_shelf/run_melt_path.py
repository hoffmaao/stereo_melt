...
Caveats specific to Beardmore_Shelf:
- Velocity is the static MEaSUREs phase map (NSIDC-0754; ITS_LIVE has no
  grounding-zone coverage here). Static velocity inflates trajectory-solver
  flux on NONSTATIONARY fast ice (PIG 2026-07-03 finding); the Ross-sector
  outlet is comparatively steady, so flag the caveat in write-ups rather
  than blocking on it.
- Default resolution is the native config.RES stack; pass ``--res N`` for a
  ``beardmore_shelf_stack_<N>m`` variant if one is built later.
...
def load_stack_res(res_m: int, tag: str | None = None) -> xr.DataArray:
    prefix = (
        "beardmore_shelf_stack"
        if res_m == config.RES
        else f"beardmore_shelf_stack_{res_m}m"
    )
    if tag:
        prefix += f"_{tag}"
...
def main() -> None:
    ...
    p.add_argument("--res", type=int, default=config.RES,
                   help=f"stack resolution suffix to load (default {config.RES})")
