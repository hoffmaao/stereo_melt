"""Run the Dotson + Crosson Lagrangian melt-rate pipeline on the built stack.

Stage 4+ of the Dotson + Crosson pipeline. Loads the already-built DEM stack,
...
returned Dataset plus QC plots to ``dotson_crosson/figures/``.

Run:

    python -m dotson_crosson.run_melt

Assumes :mod:`dotson_crosson.build_stack` has already produced
``processed/dotson_crosson_stack_<start>_<end>.nc``.
"""
...
def load_velocity_on_grid(stack: xr.DataArray) -> tuple[xr.DataArray, xr.DataArray, str]:
    ...
    Velocity source is selected by the ``DC_VELOCITY`` env var:
    ...
    """
    requested = os.environ.get("DC_VELOCITY", "measures").strip().lower()
    source = None
...
def plot_inputs(stack, vx, vy, a_dot, firn, out_path: Path) -> None:
    ...
    fig.suptitle("Dotson + Crosson melt-rate inputs", fontsize=12)
...
def plot_melt_comparison(
    ...
    # decades outward); the difference and flux-divergence panels below stay
    # linear -- they are not melt rates.
    mcmap = melt_cmap()
    ...
    fig.suptitle(
        f"Dotson + Crosson melt rate — {config.START_TIME} to {config.END_TIME}",
        fontsize=12,
    )
    fig.savefig(out_path, dpi=140, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    # reg=1e-1 from a sweep on the real Dotson + Crosson stack: lower reg leaves
    ...
    out_nc = (
        config.RESULTS_DIR
        / f"dotson_crosson_melt_{config.START_TIME}_{config.END_TIME}.nc"
    )
