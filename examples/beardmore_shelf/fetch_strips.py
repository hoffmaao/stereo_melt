"""Search the REMA strip index for the Beardmore_Shelf AOI + study window and download.

Stage 0 of the Beardmore_Shelf pipeline. Uses ``pdemtools`` (via
``stereo_melt.io.rema``) to query the s2s041 strip index for strips that
overlap ``config.BEARDMORE_SHELF_AOI_SHP`` between ``config.START_TIME`` and
``config.END_TIME``, then downloads any missing ones to
``config.STRIPS_DIR``.

Run:

    python -m beardmore_shelf.fetch_strips
"""
...
def main() -> None:
    ...
    start = os.environ.get("BEARDMORE_SHELF_FETCH_START", config.START_TIME)
    end = os.environ.get("BEARDMORE_SHELF_FETCH_END", config.END_TIME)

    with fiona.open(config.BEARDMORE_SHELF_AOI_SHP) as src:
        polygon = shape(next(iter(src))["geometry"])

    print(f"AOI: {config.SHELF}, bounds={polygon.bounds}, area={polygon.area / 1e6:.1f} km^2")
